...
class organism(): # Classe que armazenará todas as informações dos indivíduos
    ...
    # UPDATE posição
    def update_pos(self, settings): # Atualiza a posição com base na velocidade e orientação
        dx = self.v * cos(radians(self.r)) * settings['dt'] # Multiplica velocidade pelo radiano do cosseno e pelo intervalo de tempo
        dy = self.v * sin(radians(self.r)) * settings['dt'] # Multiplica velocidade pelo radiano do seno e pelo intervalo de tempo
        self.x += dx
        self.y += dy
...
def evolve(settings, organisms_old, gen):
    ...
    elitism_num = int(floor(settings['elitism'] * settings['pop_size'])) # Calcula o número de individuos selecionados pelo elitismo
    new_orgs = settings['pop_size'] - elitism_num # Calcula o número de seres restantes para completar a próxima geração

    #--- Pega a avaliação da geração atual
    ...
    stats['AVG'] = stats['SUM'] / stats['COUNT'] # Cria o array de média dos organismos
    ...
    for i in range(0, elitism_num):
        organisms_new.append(organism(settings, wih=orgs_sorted[i].wih, who=orgs_sorted[i].who, name=orgs_sorted[i].name)) # Adiciona os melhores organismos ao array utilizando elitismo
    ...
    return organisms_new, stats # Retorna a nova população
